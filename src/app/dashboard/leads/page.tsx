            ...
            {statusOptions.map(({ value, label }) => (
              ...
              >
                ...
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${
                  statusFilter === value ? 'bg-[#00FF41]/10 text-[#00FF41]' : 'bg-gray-200 text-gray-500'
                }`}>
                  {counts[value]}
                </span>
              </button>
                      ...
                      <td>
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-[#00FF41]/10 flex items-center justify-center flex-shrink-0">
                            <span className="text-[13px] font-bold text-[#00FF41]">{lead.name.charAt(0)}</span>
                          </div>
                          <div>
                        ...
                        <div className="relative" data-status-dropdown>
                          ...
                          {openStatusId === lead.id && (
                            <div
                              ...
                              className="absolute z-20 left-0 top-8 bg-white dark:bg-[#111114] border border-gray-200 dark:border-[#181819] rounded-lg shadow-lg dark:shadow-black/40 py-1 min-w-[150px]"
                            >
                              {(['novo', 'contatado', 'qualificado', 'descartado', 'convertido'] as LeadStatus[]).map(s => (
                                <button
                                  ...
                                  onKeyDown={e => { if (e.key === 'Escape') setOpenStatusId(null) }}
                                  className={`w-full text-left px-3 py-2 text-[12px] hover:bg-gray-50 dark:hover:bg-[#181819] cursor-pointer transition-colors ${lead.status === s ? 'font-semibold text-[#00FF41] dark:text-[#00FF41]' : 'text-gray-700 dark:text-[#D1FAE5]'}`}
                                >
                      ...
                      <td>
                        <div className="flex items-center gap-0.5">
                          ...
                          {lead.status !== 'convertido' && lead.status !== 'descartado' && (
                            <Button variant="ghost" size="sm" className="text-[#00FF41] hover:bg-[#00FF41]/10" onClick={() => setConvertModal(lead)}>
                              <UserPlus size={13} />
                            </Button>
                          )}
                          <button
      ...
      <Modal isOpen={!!convertModal} onClose={() => setConvertModal(null)} title="Converter em Cliente" size="sm">
        {convertModal && (
          <div className="space-y-4">
            ...
            <div className="bg-[#F8FBF9] border border-[#00FF41]/20 rounded-lg p-3 text-sm space-y-1.5">
