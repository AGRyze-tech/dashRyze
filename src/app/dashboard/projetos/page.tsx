...
function DetailRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="text-[11px] font-medium text-gray-400 dark:text-[#017e26] uppercase tracking-wide mb-0.5">{label}</p>
      {children}
    </div>
  )
}
...
const ProjectCard = memo(function ProjectCard({
  ...
  return (
    <div className="bg-white dark:bg-[#1c1c1f] border border-gray-200 dark:border-[#2a2a2e] rounded-xl p-4 shadow-sm dark:shadow-black/30 hover:shadow-md transition-all duration-200 group">
      <div className="flex items-start justify-between mb-2.5">
        ...
        <div className="flex items-center gap-1 ml-2 flex-shrink-0">
          <span className="text-[10px] font-medium text-gray-400 dark:text-[#01992e] bg-gray-100 dark:bg-[#252528] px-1.5 py-0.5 rounded">
          ...
          <button
            ...
            className="p-1 rounded hover:bg-blue-50 dark:hover:bg-blue-900/20 text-gray-300 dark:text-[#33333a] hover:text-blue-500 dark:hover:text-blue-400 transition-colors opacity-0 group-hover:opacity-100"
          >
            <Eye size={11} />
          </button>
          <button
            ...
            className="p-1 rounded hover:bg-gray-100 dark:hover:bg-[#252528] text-gray-300 dark:text-[#33333a] hover:text-gray-600 dark:hover:text-[#01992e] transition-colors opacity-0 group-hover:opacity-100"
          >
            <Pencil size={11} />
          </button>
          <button
            ...
            className="p-1 rounded hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-300 dark:text-[#33333a] hover:text-red-500 dark:hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"
          >
            <Trash2 size={11} />
          </button>
        </div>
      ...
      <div className="flex items-center mt-3">
        ...
        <span className="text-[11px] text-gray-500 dark:text-[#01992e] capitalize">{project.responsible}</span>
      </div>

      {active && (
        <div className={`flex items-center gap-1.5 mt-3 pt-3 border-t border-gray-100 dark:border-[#2a2a2e] ${over ? 'text-red-500 dark:text-red-400' : warn ? 'text-amber-500 dark:text-amber-400' : 'text-gray-400 dark:text-[#017e26]'}`}>
      ...
      {project.url && (
        <a
          ...
          className="flex items-center gap-1 mt-2 text-[11px] text-[#01CA3C] hover:underline"
            ...
            {columns.map(col => {
              ...
              return (
                <div key={col.status} className="flex items-center gap-1.5">
                  ...
                  <span className="text-[12px] text-gray-500 dark:text-[#01992e]">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4" style={{ minHeight: '70vh' }}>
          {columns.map(col => {
            ...
            return (
              <div
                key={col.status}
                className={`flex-shrink-0 w-[270px] rounded-xl transition-all duration-150 bg-gray-100 dark:bg-[#1e1e22] ${isOver ? 'ring-2 ring-[#01CA3C]/40 !bg-[#01CA3C]/5' : ''}`}
                onDragOver={e => { e.preventDefault(); setDragOver(col.status) }}
                onDragLeave={() => setDragOver(null)}
                onDrop={() => handleDrop(col.status)}
              >
                <div className="flex items-center justify-between px-3 py-3 border-b border-gray-200/70 dark:border-[#2a2a2e]">
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full" style={{ background: col.color }} />
                    <span className="text-[12px] font-semibold text-gray-700 dark:text-[#D1FAE5]">{col.label}</span>
                    <span className="text-[11px] text-gray-400 dark:text-[#017e26] bg-white dark:bg-[#1c1c1f] border border-gray-200 dark:border-[#33333a] rounded-full px-1.5 py-px font-medium">
                      {colProjects.length}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-2.5 space-y-2.5 min-h-[200px]">
                  ...
                  {colProjects.length === 0 && (
                    <div className="border-2 border-dashed border-gray-200 dark:border-[#2a2a2e] rounded-xl h-20 flex items-center justify-center">
                      <span className="text-[11px] text-gray-400 dark:text-[#33333a]">Soltar aqui</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              {viewTarget.url && (
                <div className="col-span-2">
                  <DetailRow label="URL">
                    <a href={viewTarget.url.startsWith('http') ? viewTarget.url : `https://${viewTarget.url}`} target="_blank" rel="noopener noreferrer" className="text-[13px] text-[#01CA3C] hover:underline flex items-center gap-1">
