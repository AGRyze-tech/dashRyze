          ...
          <div className="col-span-1 space-y-4">
            <Card>
              ...
              <div className="flex flex-col items-center text-center mb-5">
                <div className="w-16 h-16 rounded-2xl bg-[#32B86A]/10 dark:bg-[#32B86A]/15 flex items-center justify-center mb-3">
                  <span className="text-2xl font-bold text-[#32B86A]">{client.name.charAt(0)}</span>
                </div>
                <h2 className="font-semibold text-gray-900 dark:text-[#F8FBF9] text-base leading-tight mb-0.5">{client.name}</h2>
                <p className="text-sm text-gray-500 dark:text-[#3E9E60] mb-2">{client.specialty}</p>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 dark:border-[#111B14] space-y-2">
                {client.closed_at && (
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-medium text-gray-600 dark:text-[#3E9E60]">{formatDate(client.closed_at)}</span>
                  </div>
                )}
                {client.delivery_date && (
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-medium text-gray-600 dark:text-[#3E9E60]">{formatDate(client.delivery_date)}</span>
                  </div>
                )}
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-1.5 text-gray-400 dark:text-[#2A5C3C]">
                  ...
                  <span className="font-medium text-gray-600 dark:text-[#3E9E60]">{formatDate(client.created_at)}</span>
                </div>
              </div>
            </Card>
                    ...
                    <div key={project.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 dark:hover:bg-[#0C150E] transition-colors">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[11px] text-gray-400 dark:text-[#3E9E60]">
                      ...
                      <div className="text-right flex-shrink-0 space-y-0.5">
                        ...
                        {project.start_date && (
                          <p className="text-[11px] text-gray-400 dark:text-[#3E9E60]">Início: {formatDate(project.start_date)}</p>
                        )}
                        {project.deadline && (
                          <p className="text-[11px] text-gray-400 dark:text-[#3E9E60]">Prazo: {formatDate(project.deadline)}</p>
                        )}
                        {project.url && (
                          <a href={`https://${project.url}`} target="_blank" rel="noopener noreferrer"
                            className="text-[11px] text-[#32B86A] dark:text-[#4EE88A] hover:underline flex items-center gap-1 justify-end">
