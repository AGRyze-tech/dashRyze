    ...
    <aside className={cn(
      ...
      <div className="px-5 py-5 border-b border-[#2a2a2e]">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center flex-shrink-0 p-1 ring-1 ring-[#01CA3C]/30 shadow-[0_0_14px_rgba(64,145,108,0.28)]">
            <Image
          ...
          <div className="leading-none">
            ...
            <div className="text-[10px] font-medium text-[#017e26] tracking-[0.15em] uppercase">Systems</div>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto space-y-3">
        {navGroups.map(group => (
          <div key={group.label}>
            <p className="px-3 pb-1 text-[10px] font-semibold text-[#33333a] uppercase tracking-[0.15em]">{group.label}</p>
        ...
        {userRole === 'admin' && (
          <div>
            <p className="px-3 pb-1 text-[10px] font-semibold text-[#33333a] uppercase tracking-[0.15em]">Administração</p>
            <div className="space-y-0.5">
      ...
      <div className="px-3 py-4 border-t border-[#2a2a2e]">
        <div className="flex items-center gap-3 px-3 py-2.5 mb-1">
          <div className="w-7 h-7 rounded-full bg-[#01CA3C] flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
            {userRole === 'admin' ? 'I' : 'V'}
          </div>
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[11px] text-[#017e26] capitalize">{userRole}</div>
          </div>
          <button
            type="button"
            onClick={toggleTheme}
            aria-label={theme === 'dark' ? 'Modo claro' : 'Modo escuro'}
            className="p-1.5 rounded-md text-[#017e26] hover:text-[#01992e] hover:bg-[#2a2a2e] transition-colors flex-shrink-0"
          >
            {theme === 'dark' ? <Sun size={14} /> : <Moon size={14} />}
          </button>
        </div>
        <button type="button" onClick={handleSignOut} className="nav-item w-full text-[#017e26] hover:text-red-400 hover:bg-red-400/5">
